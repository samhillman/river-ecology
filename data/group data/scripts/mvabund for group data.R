...
inverts[is.na(inverts)] <- 0
head(inverts)

inverts_spp <- mvabund(inverts[,4:43])
head(inverts_spp)

par(mar=c(2,10,2,2)) # adjusts the margins
boxplot(inverts[,4:43],horizontal = TRUE,las=2, main="Abundance")
...
mod2 <- manyglm(inverts_spp ~ inverts$year, family="negative_binomial")
plot(mod2)
...
mod3 <- manyglm(inverts_spp ~ inverts$site*inverts$year, family="negative_binomial")
anova(mod3)

#so difference between site, but not temp? Eh?

summary.manyglm(mod3)
